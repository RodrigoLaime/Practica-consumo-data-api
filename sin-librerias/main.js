...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  userList.innerHTML = "<h1>Loading</h1>";/* antes de que se cargen los datos se muestra loading */

  const data = await loadUsers();
  console.log(data);
  users = data.data;/* guardamos los datos en el array user */
  renderUsers(users);/* y renderizamos en el navegador los datos directamente del arreglo users */
});

/* evento al input  */
input.addEventListener("keyup", (e) => {
  /* keyup da informacion de lo que esta tipeando el usuario en el input */
  const newUsers = users.filter((user) =>/* filter crea un array con todos los que cumplen con la condicion */
    `${user.firstname.toLowerCase()} ${user.lastname.toLowerCase()} ${user.email
        .toLowerCase()}`.includes(/* includes te indica si existe o no */
...
const createUserItems = (users) =>
  users.map((user) => `<li class="lista">${user.firstname}</li> 
    <li class="lista">${user.lastname}</li> 
    <li class="lista">${user.email}</li>`)
